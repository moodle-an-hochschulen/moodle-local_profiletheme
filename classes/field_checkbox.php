...
/**
 * Local plugin "Profile field based theme delivery" - Handles checkbox field types
 *
...
class field_checkbox extends field_base {
    ...
    protected function add_form_field_internal(MoodleQuickForm $mform, $id) {
        ...
        $opts = [
            1 => get_string('yes'),
            0 => get_string('no'),
            self::MATCH_ISDEFINED => get_string('match_defined', 'local_profiletheme'),
            self::MATCH_NOTDEFINED => get_string('match_notdefined', 'local_profiletheme'),
        ];

        $label = $mform->createElement('static', "matchlabel[$id]", '', get_string('match_exact', 'local_profiletheme').
                '<span class="pr-2"></span>');
        $sel = $mform->createElement('select', "matchvalue[$id]", get_string('matchvalue', 'local_profiletheme'), $opts,
                ['class' => 'pr-2']);
